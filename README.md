# **EVOTEM HR 🦍** [![LGPL License](https://img.shields.io/badge/license-LGPL-green.svg)](https://www.gnu.org/licenses/lgpl-3.0)  [![Docker](https://img.shields.io/badge/Docker-EVOTEM HR-blue?logo=docker)](https://hub.docker.com/r/evotem_hr/evotem_hr)

**EVOTEM HR** is a Free and Open Source HRMS (Human Resource Management System) Software designed to streamline HR processes and enhance organizational efficiency.

![EVOTEM HR Screenshot](https://github.com/evotem_hr-opensource/evotem_hr/assets/131998600/1317bd0a-03a8-40be-8fb2-ecb655bb5c13)
...
5. **Configure PostgreSQL Database and User**:
   ...
   - Create a new role and database:
     ```sql
     CREATE ROLE evotem_hr LOGIN PASSWORD 'evotem_hr';
     CREATE DATABASE evotem_hr_main OWNER evotem_hr;
     \q
     ```
...
4. **Configure PostgreSQL Database and User**:
   - Create a database and user:
     ```bash
     createdb evotem_hr_main
     createuser evotem_hr
     psql -c "ALTER USER evotem_hr WITH PASSWORD 'evotem_hr';"
     ```
...
Follow the steps below to install **EVOTEM HR** on your system. EVOTEM HR is compatible with **Ubuntu**, **Windows**, and **macOS**.
...
```bash
sudo git init
sudo git remote add evotem_hr https://evotem_hr-opensource@github.com/evotem_hr-opensource/evotem_hr.git
sudo git pull evotem_hr master
```

#### **Windows**
```powershell
git init
git remote add evotem_hr https://evotem_hr-opensource@github.com/evotem_hr-opensource/evotem_hr.git
git pull evotem_hr master
```
...
2. Create and activate the virtual environment:
   ```bash
   python3 -m venv evotem_hrvenv
   source evotem_hrvenv/bin/activate
   ```
...
1. Create and activate the virtual environment:
   ```powershell
   python -m venv evotem_hrvenv
   .\evotem_hrvenv\Scripts\activate
   ```
...
If everything is configured correctly, you should be able to access your EVOTEM HR app at **http://localhost:8000**.
![Initialize Database in EVOTEM HR HRMS](https://www.evotem_hr.com/wp-content/uploads/2024/12/how-to-initialize-the-database-in-evotem_hr-hrms-step-by-step-1-1024x576.png)


#### **Initial Setup**
From the login page, you will have two options:
1. **Initialize Database**: Use this option to initialize the EVOTEM HR database by creating a super admin, headquarter company, department, and job position. Authenticate using the `DB_INIT_PASSWORD` specified in the `.env` file.
...
If you wish to run the EVOTEM HR application on a different port, specify the port number after the `runserver` command. For example:
```bash
...
[EVOTEM HR](https://www.evotem_hr.com/) is an open-source HRMS solution designed to simplify HR operations and improve organizational efficiency.

---

This README provides a comprehensive guide to installing and setting up EVOTEM HR on various platforms. If you encounter any issues, feel free to reach out to the EVOTEM HR community for support. Happy coding! 🚀
