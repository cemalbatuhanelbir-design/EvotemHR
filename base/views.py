...
from horilla import evotem_hr_apps
...
from horilla.evotem_hr_settings import (
    APPS,
    DB_INIT_PASSWORD,
...
from horilla.methods import get_evotem_hr_model_class, remove_dynamic_url
...
def initialize_database(request):
    ...
    if initialize_database_condition():
        ...
        return render(request, "initialize_database/evotem_hr_user.html")
    else:
        return redirect("/")
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request, "initialize_database/evotem_hr_user_signup.html")
        ...
        return render(
            request,
            "initialize_database/evotem_hr_company.html",
...
def initialize_database_company(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "initialize_database/evotem_hr_department.html",
                {"form": DepartmentForm(initial={"company_id": company})},
            )
    return render(request, "initialize_database/evotem_hr_company.html", {"form": form})
...
def initialize_database_department(request):
    ...
    return render(
        request,
        "initialize_database/evotem_hr_department_form.html",
...
def initialize_database_job_position(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "initialize_database/evotem_hr_job_position_form.html",
            {
                "form": form,
    ...
    return render(
        request,
        "initialize_database/evotem_hr_job_position.html",
...
class EVOTEM HRPasswordResetView(PasswordResetView):
    """
    EVOTEM HR View for Reset Password
    """
...
class EmployeePasswordResetView(PasswordResetView):
    """
    EVOTEM HR View for Employee Reset Password
    """
...
def two_factor_auth(request):
    ...
    if not evotem_hr_apps.TWO_FACTORS_AUTHENTICATION:
        return redirect("/")
...
def object_delete(request, obj_id, **kwargs):
    ...
    if apps.is_installed("pms") and redirect_path == "/pms/filter-key-result/":
        KeyResult = get_evotem_hr_model_class(app_label="pms", model="keyresult")
...
def mail_server_test_email(request):
    ...
    white_labelling = getattr(evotem_hr_apps, "WHITE_LABELLING", False)
    image_path = path.join(settings.STATIC_ROOT, "images/ui/evotem_hr-logo.png")
    company_name = "EVOTEM HR"
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subject = _("Test mail from EVOTEM HR")
...
@login_required
@hx_request_required
@permission_required("base.change_evotem_hrmailtemplate")
def view_mail_template(request, obj_id):
    """
    This method is used to display the template/form to edit
    """
    template = EVOTEM HRMailTemplate.objects.get(id=obj_id)
...
@login_required
@hx_request_required
@permission_required("base.add_evotem_hrmailtemplate")
...
@login_required
@permission_required("base.delete_evotem_hrmailtemplate")
def delete_mail_templates(request):
    ids = request.GET.getlist("ids")
    result = EVOTEM HRMailTemplate.objects.filter(id__in=ids).delete()
...
def employee_shift_view(request):
    ...
    if apps.is_installed("attendance"):
        GraceTime = get_evotem_hr_model_class(app_label="attendance", model="gracetime")
...
def general_settings(request):
    ...
    if apps.is_installed("payroll"):
        PayrollSettings = get_evotem_hr_model_class(
            app_label="payroll", model="payrollsettings"
        )
        EncashmentGeneralSettings = get_evotem_hr_model_class(
...
@login_required
def get_evotem_hr_installed_apps(request):
