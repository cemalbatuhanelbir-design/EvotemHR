...
from base.evotem_hr_company_manager import EVOTEM HRCompanyManager
from horilla import evotem_hr_middlewares
from horilla.evotem_hr_middlewares import _thread_locals
from horilla.models import EVOTEM HRModel, upload_path
from horilla_audit.models import EVOTEM HRAuditInfo, EVOTEM HRAuditLog
...
class Department(EVOTEM HRModel):
    """
    Department model
    """
    ...
    objects = EVOTEM HRCompanyManager()
...
class JobPosition(EVOTEM HRModel):
    """
    JobPosition model
    """
    ...
    objects = EVOTEM HRCompanyManager("department_id__company_id")
...
class JobRole(EVOTEM HRModel):
    ...
    objects = EVOTEM HRCompanyManager("job_position_id__department_id__company_id")
...
class RotatingWorkType(EVOTEM HRModel):
    """
    ...
    objects = EVOTEM HRCompanyManager("employee_id__employee_work_info__company_id")
...
class RotatingWorkTypeAssign(EVOTEM HRModel):
    """
...
class EmployeeType(EVOTEM HRModel):
    """
    EmployeeType model
    """
...
class EmployeeShift(EVOTEM HRModel):
    """
    EmployeeShift model
    """
    ...
    objects = EVOTEM HRCompanyManager("employee_shift__company_id")
...
class EmployeeShiftSchedule(EVOTEM HRModel):
    """
    ...
    auto_punch_out_time = models.TimeField(
        ...
        help_text=_(
            "Time at which the evotem_hr will automatically check out the employee attendance if they forget."
        ),
    )
    company_id = models.ManyToManyField(Company, blank=True, verbose_name=_("Company"))

    objects = EVOTEM HRCompanyManager("shift_id__employee_shift__company_id")
...
class RotatingShift(EVOTEM HRModel):
    """
    RotatingShift model
    """
...
class RotatingShiftAssign(EVOTEM HRModel):
    """
...
class WorkTypeRequest(EVOTEM HRModel):
    """
    ...
    def clean(self):
        request = getattr(evotem_hr_middlewares._thread_locals, "request", None)
...
class WorkTypeRequestComment(EVOTEM HRModel):
    """
...
class ShiftRequest(EVOTEM HRModel):
    """
    ShiftRequest model
    """
...
class ShiftRequestComment(EVOTEM HRModel):
    """
...
class Tags(EVOTEM HRModel):
    title = models.CharField(max_length=30)
    color = models.CharField(max_length=30)
    company_id = models.ForeignKey(
        Company, null=True, editable=False, on_delete=models.PROTECT
    )
    objects = EVOTEM HRCompanyManager(related_company_field="company_id")
...
class EVOTEM HRMailTemplate(EVOTEM HRModel):
...
class DynamicEmailConfiguration(EVOTEM HRModel):
    """
...
class MultipleApprovalCondition(EVOTEM HRModel):
...
class Announcement(EVOTEM HRModel):
    """
...
class AnnouncementComment(EVOTEM HRModel):
    """
...
class DashboardEmployeeCharts(EVOTEM HRModel):
    from employee.models import Employee
...
class TrackLateComeEarlyOut(EVOTEM HRModel):
...
class CompanyLeaves(EVOTEM HRModel):
...
class PenaltyAccounts(EVOTEM HRModel):
    """
